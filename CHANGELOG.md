...
- **Projections** — daily / weekly / monthly token savings + estimated USD cost
- **Today compression check** — raw vs optimized tokens with exact counts for verification
- `src/calculator.ts` — pure projection + compression snapshot logic
- Unit tests for calculator (`npm test`)

### Changed

- Raw / Optimized cards show full comma-separated numbers (not abbreviated K)
- Dashboard labels clarify before/after compression
